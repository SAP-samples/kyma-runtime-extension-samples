...
func (s *Server) AddOrder(order_id string, description string, status string, email string) ([]Order, error) {
	tsql := fmt.Sprintf("INSERT INTO Orders(order_id, description, status, email) VALUES(@p1,@p2,@p3,@p4);")
	_, err := s.exec(tsql, order_id, description, status, email)
	if err != nil {
		return nil, err
	}

	tsql = fmt.Sprintf("SELECT * FROM Orders WHERE order_id=@p1;")
	return s.query(tsql, order_id, description, status, email)
}

func (s *Server) EditOrder(order_id string, description string, status string, email string) (RowsAffected, error) {
	tsql := fmt.Sprintf("UPDATE Orders SET description=@p2, status=@p3, email=@p4 WHERE order_id=@p1")
	return s.exec(tsql, order_id, description, status, email)
}
...
func (s *Server) query(tsql string, args ...interface{}) ([]Order, error) {
	...
	for rows.Next() {
		err := rows.Scan(&order.Orderid, &order.Description, &order.Status, &order.Email, &order.Created)
		if err != nil {
			return nil, err
		}

...
import (
	...
	"github.com/sap-samples/kyma-runtime-extension-samples/dsagtt22/ordermicroservice/api-mssql-go/internal/db"
)
...
func (s *server) EditOrder(w http.ResponseWriter, r *http.Request) {
	...
	rowsEffected, err := s.db.EditOrder(order.Orderid, order.Description, order.Status, order.Email)
...
func (s *server) AddOrder(w http.ResponseWriter, r *http.Request) {
	...
	orders, err := s.db.AddOrder(order.Orderid, order.Description, order.Status, order.Email)
